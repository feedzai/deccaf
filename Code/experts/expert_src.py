...
class MLModelExpert(AbstractExpert):
    ...
    def __init__(self, fitted_model, threshold = None, init_score = None):
    ...
    def predict(self, X, **kwargs):  # kwargs not used (compatibility purposes)
        if self.init_score is None:
            if self.threshold is None:
                ...
                y_pred = (self.model.predict_proba(X)[:, 1].squeeze() >= self.threshold).astype(int)
        else:
            if self.threshold is None:
                y_pred = output(X,self.model, self.init_score)
            else:
                y_pred = (output(X,self.model, self.init_score) >= self.threshold).astype(int)
...
class SigmoidExpert(AbstractExpert):
    ...
    def fit(self, X, y, score_col, protected_col):
        ...
        print(f'Target FNR = {self.fnr}')
        print(f'Target FPR = {self.fpr}')
        self.fnr_beta = self.fnr
        # FALSE POSITIVES
        self.fpr_beta = self.fpr

        X.columns.get_loc(protected_col)
