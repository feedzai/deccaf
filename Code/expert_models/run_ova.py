    ...
    for l in costs:
        ...
        for train in os.listdir(f'../../Data_and_models/testbed/{scen}-l_{l}/train_alert'):
            ...
            for expert in experts:
                ...
                if not (os.path.exists(f'../../Data_and_models/expert_models/ova/{scen}-l_{l}/{train}/{expert}/')):
                    os.makedirs(f'../../Data_and_models/expert_models/ova/{scen}-l_{l}/{train}/{expert}/')
                
                if not (os.path.exists(f'../../Data_and_models/expert_models/ova/{scen}-l_{l}/{train}/{expert}/best_model.pickle')):
                    opt = hpo.HPO(train_x,val_x,train_y,val_y,train_w, val_w, method = 'TPE', path = f'../../Data_and_models/expert_models/ova/{scen}-l_{l}/{train}/{expert}/')
...
for scen in os.listdir('../../Data_and_models/data/alerts/'):
    ...
    for l in costs:
        ...
        for env in os.listdir(f'../../Data_and_models/expert_models/ova/{scen}-l_{l}'):
            table = pd.DataFrame(index = test.index, columns = os.listdir(f'../../Data_and_models/expert_models/ova/{scen}-l_{l}/{env}'))
            for expert in os.listdir(f'../../Data_and_models/expert_models/ova/{scen}-l_{l}/{env}'):
                
                with open(f"../../Data_and_models/expert_models/ova/{scen}-l_{l}/{env}/{expert}/best_model.pickle", "rb") as input_file:
