...
train['model_score'] = experts_pred.loc[train.index, 'model#0']
...
for (batch_scheme, capacity_scheme), (train_batches, train_capacity) in train_envs.items():
    ...
    env_train.to_parquet(
        f"{cfg['output_paths']['train_dir_alert']}{batch_scheme}#{capacity_scheme}/train.parquet"
    )
